...
class MYPDF extends TCPDF {
    ...
    public function ColoredTable($header) {
        ...
        $sql = "SELECT b. *, a.name, a.age, a.teacher_incharge, a.long_term_target FROM learner a JOIN learner_program b ON b.learnerID = a.learnerID WHERE a.learnerID = '$learnerID'";
        ...
        $this->SetY($this->GetY());
        $this->Cell(0, 6, "Date:  $program", 0, 1, 'L');
...
//target program table 
    ...
    $sql = "SELECT * FROM learner_targetprogram";
        ...
        $this->Cell(0, 6, 'Detailed Activities (Weekly Targets):', 0, 1, 'L');
          ...
          $this->SetFont('times','B', 10);
          $this->Cell($tableWidth, 6, '1st Month: ', 1, 1, 'C');
          ...
          //$this->Cell($tableWidth, 6, '1st Month: '.$month1_date_to,'-'.$month1_date_to 1, 1, 'C');
          $this->SetFont('times','B', 10);
          $this->Cell($tableWidth, 6, '2nd Month: ', 1, 1, 'C');
        ...
        $this->SetXY($x, $y);
        $this->MultiCell($columnWidth, 65, $month2_targetprogram, 1, 'L', 0, 0);

        
        $this->SetXY($x + $columnWidth, $y);
        $this->MultiCell($columnWidth, 65, $month2_week1, 1, 'L', 0, 0);

      
        $this->SetXY($x + 2 * $columnWidth, $y);
        $this->MultiCell($columnWidth, 65, $month2_week2, 1, 'L', 0, 0);

       
        $this->SetXY($x + 3 * $columnWidth, $y);
        $this->MultiCell($columnWidth, 65, $month2_week3, 1, 'L', 0, 0);

        
        $this->SetXY($x + 4 * $columnWidth, $y);
        $this->MultiCell($columnWidth, 65, $month2_week4, 1, 'L', 0, 1);
          ...
          $this->SetFont('times','B', 10);
          $this->Cell($tableWidth, 6, '3rd Month: ', 1, 1, 'C');
