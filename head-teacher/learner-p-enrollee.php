        ...
        <div class="row">
          ...
          <table id="example" class="table table-striped border" style="width:100%;font-size:12px;">
            ...
            <tbody>
              <?php
                $count=1;
                $query = "SELECT * FROM `learner` WHERE archive='0' and trigger_ht='1' and trigger_dr_ass='1' and (approved='0' || approved='1')";
                  ...
                  if(mysqli_num_rows($query_run) > 0){
                    foreach($query_run as $row){
                      if($row['trigger_dr_ass']=="0"){
                        ...
                        $trigger_color = "0, 102, 0";
                        $trigger_dr_ass = "Completed";
                      }
                      
                      if($row['approved']=="0"){
                        $approved_color = "255, 0, 0";
                        $approved = "Pending";
                      }
                      else if($row['approved']=="1"){
                        $approved_color = "0, 0, 255";
                        $approved = "Enrolled";
                      }
                      else{
                        $approved_color = "250, 0, 0";
                        $approved = "Reject";
                      }
                ?>
                      <tr>
                          ...
                          <td class="text-center"  style="color:rgba(<?= $approved_color; ?>);"><?= $approved; ?></td>
                          <td class="text-center p-0 pt-1" style="width:120px;">
                            <a href="evaluate-enrollee.php?id=<?= $row['learnerID']; ?>" class="btn p-1 ps-2 pe-2 border-0 text-white" style="background-color:rgba(4, 3, 70);">Evaluate Enrollee</a>
                          </td>
                      </tr>
