...
// Kunin lahat ng records sa buwang ito
...
while ($row = mysqli_fetch_assoc($query_run)) {
    $week = date('W', strtotime($row['date_']));
    $weekly_data[$week][] = $row;
}

// Kunin lahat ng week numbers sa range
...
?>
            ...
            <thead class="table-light">
                <tr>
                    ...
                    <?php foreach ($weekly_data as $week_num => $entries): ?>
                        <th class="text-center">Week <?= $week_num; ?></th>
                    <?php endforeach; ?>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <?php foreach ($weekly_data as $entries): 
                ...
                <tr><td class="ps-4">Body Awareness</td>
                    <?php foreach ($weekly_data as $entries): 
                        $sum = array_sum(array_column($entries, 'body'));
                        echo "<td class='text-center'>" . round($sum / count($entries), 2) . "</td>";
                    endforeach; ?>
                </tr>
                <tr><td class="ps-4">Confidence</td>
                    <?php foreach ($weekly_data as $entries): 
                        $sum = array_sum(array_column($entries, 'confidence'));
                        echo "<td class='text-center'>" . round($sum / count($entries), 2) . "</td>";
                    endforeach; ?>
                </tr>
                <tr><td class="ps-4">Sound Tolerance</td>
                    <?php foreach ($weekly_data as $entries): 
                        $sum = array_sum(array_column($entries, 'sound'));
                        echo "<td class='text-center'>" . round($sum / count($entries), 2) . "</td>";
                    endforeach; ?>
                </tr>
                <tr>
                            ...
                            <div class="modal-dialog modal-xl">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <iframe src="pdf-monthly.php?id=<?= htmlspecialchars($learnerID) ?>&start=<?= htmlspecialchars($start_date) ?>&end=<?= htmlspecialchars($end_date) ?>" 
                                                width="100%" 
                                                height="600px" 
...
<?php
// Calculate the monthly averages for each intervention
$massage_avg = 0;
$action_song_avg = 0;
$sensory_integration_avg = 0;

// Iterate through each week and calculate average for each intervention
foreach ($weekly_data as $week_num => $entries) {
    // Massage: Average of emotion + sensitivity
    $massage_avg += array_reduce($entries, function($carry, $row) {
        return $carry + (($row['emotion'] + $row['sensitivity']) / 2);
    }, 0) / count($entries);
    
    // Action Song: Average of body + confidence + sound
    $action_song_avg += array_reduce($entries, function($carry, $row) {
        return $carry + (($row['body'] + $row['confidence'] + $row['sound']) / 3);
    }, 0) / count($entries);

    // Sensory Integration: Average of gross motor skills and others
    $sensory_integration_avg += array_reduce($entries, function($carry, $row) {
        return $carry + (($row['walking'] + $row['jumping']+ $row['crawling']+ $row['hopping']+ $row['muscle_strength']+ $row['muscle']+ $row['balance']+ $row['bilateral']+ $row['gross']) / 9);
    }, 0) / count($entries);
}

// Now calculate the final monthly average
$total_weeks = count($weekly_data);
$massage_avg = $massage_avg / $total_weeks;
$action_song_avg = $action_song_avg / $total_weeks;
$sensory_integration_avg = $sensory_integration_avg / $total_weeks;
?>
...
window.onload = function () {
    ...
    title: {
        text: "Intervention Progress for the Month"
    },
    axisY: {
        title: "Average Score"
    },
    data: [{
        type: "bar", 
        dataPoints: [
            { label: "Massage", y: <?= round($massage_avg, 2); ?> },  
            { label: "Action Song", y: <?= round($action_song_avg, 2); ?> },  
            { label: "Sensory Integration", y: <?= round($sensory_integration_avg, 2); ?> },  
            { label: "One on One", y: <?= round($massage_avg, 2); ?> },
			{ label: "Overall Socialization and Communication", y: <?= round($massage_avg, 2); ?>}
        ]
    }]
});
chart.render();
