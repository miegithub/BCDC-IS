  ...
  <div class="row">
                        ...
                        <div class="container p-3 card shadow-sm">
                          <div class="row">
                            ...
                            <?php
                              ...
                              // Kunin lahat ng dates between start and end
                              $period = new DatePeriod(
                                  new DateTime($start_date),
                                  new DateInterval('P1D'),
                                  (new DateTime($end_date))->modify('+1 day') // kasama end date
                              );

                              // Kunin ang records
                              $query = "SELECT * FROM learner_progress WHERE learnerID='$learnerID' AND date_ BETWEEN '$start_date' AND '$end_date'";
                              $query_run = mysqli_query($con, $query);
    ...
    <tr>
        ...
        foreach ($period as $date) {
            ...
            if (isset($daily_data[$d])) {
                echo "<th class='text-center'>{$date->format('l')}<br>{$d}</th>";
            }
        }
        ?>
    </tr>
    ...
    <tbody>
        ...
        foreach ($period as $date) {
            ...
            if (isset($daily_data[$d])) {
                $row = $daily_data[$d];
                echo "<td class='text-center'><strong>" . round(($row['emotion'] + $row['sensitivity']) / 2, 2) . "</strong></td>";
            }
        }
        ?>
    </tr>
    <tr><td class="ps-4">Emotion Regulation</td>
        ...
        foreach ($period as $date) {
            $d = $date->format('Y-m-d');
            if (isset($daily_data[$d])) {
                echo "<td class='text-center'>" . $daily_data[$d]['emotion'] . "</td>";
            }
        }
        ?>
    </tr>
        ...
        foreach ($period as $date) {
            $d = $date->format('Y-m-d');
            if (isset($daily_data[$d])) {
                echo "<td class='text-center'>" . $daily_data[$d]['sensitivity'] . "</td>";
            }
        }
        ?>
        </tr>
        ...
        foreach ($period as $date) {
            ...
            if (isset($daily_data[$d])) {
                $row = $daily_data[$d];
                echo "<td class='text-center'><strong>" . round(($row['body'] + $row['confidence'] + $row['sound']) / 3, 2) . "</strong></td>";
            }
        }
        ?>
        ...
        foreach ($period as $date) {
            $d = $date->format('Y-m-d');
            if (isset($daily_data[$d])) {
                echo "<td class='text-center'>" . $daily_data[$d]['body'] . "</td>";
            }
        }
        ?>
        </tr>
        ...
        foreach ($period as $date) {
            $d = $date->format('Y-m-d');
            if (isset($daily_data[$d])) {
                echo "<td class='text-center'>" . $daily_data[$d]['confidence'] . "</td>";
            }
        }
        ?>

        </tr>
        <tr><td class="ps-4">Sound</td>
        <?php 
        foreach ($period as $date) {
            $d = $date->format('Y-m-d');
            if (isset($daily_data[$d])) {
                echo "<td class='text-center'>" . $daily_data[$d]['sound'] . "</td>";
            }
        }
        ?>
        </tr>
        ...
        <td class="subheader"><a href="#">C. SENSORY INTEGRATION</a></td>
                ...
                <?php 
                  foreach ($period as $date) {
                      ...
                      if (isset($daily_data[$d])) {
                          $row = $daily_data[$d];
                          echo "<td class='text-center'><strong>" . round(($row['walking'] + $row['jumping'] + $row['crawling']+ $row['hopping']+ $row['muscle_strength']+ $row['muscle']+ $row['balance']+ $row['bilateral']+ $row['gross']) / 9, 2) . "</strong></td>";
                      }
                  }
                  ?>
                </td>
        ...
        <tr>
                                        ...
                                        foreach ($period as $date) {
                                            $d = $date->format('Y-m-d');
                                            if (isset($daily_data[$d])) {
                                                echo "<td class='text-center'>" . $daily_data[$d]['gross'] . "</td>";
                                            }
                                        }
                                        ?>
                                    </tr>
                                    <tr>
                                        ...
                                        foreach ($period as $date) {
                                            $d = $date->format('Y-m-d');
                                            if (isset($daily_data[$d])) {
                                                echo "<td class='text-center'>" . $daily_data[$d]['walking'] . "</td>";
                                            }
                                        }
                                        ?>
                                    </tr>
                                    <tr>
                                        ...
                                        foreach ($period as $date) {
                                            $d = $date->format('Y-m-d');
                                            if (isset($daily_data[$d])) {
                                                echo "<td class='text-center'>" . $daily_data[$d]['jumping'] . "</td>";
                                            }
                                        }
                                        ?>
                                    </tr>
                                    <tr>
                                        ...
                                        foreach ($period as $date) {
                                            $d = $date->format('Y-m-d');
                                            if (isset($daily_data[$d])) {
                                                echo "<td class='text-center'>" . $daily_data[$d]['crawling'] . "</td>";
                                            }
                                        }
                                        ?>
                                    </tr>
                                    <tr>
                                        ...
                                        foreach ($period as $date) {
                                            $d = $date->format('Y-m-d');
                                            if (isset($daily_data[$d])) {
                                                echo "<td class='text-center'>" . $daily_data[$d]['hopping'] . "</td>";
                                            }
                                        }
                                        ?>
                                    </tr>
                                    <tr>
                                        ...
                                        foreach ($period as $date) {
                                            $d = $date->format('Y-m-d');
                                            if (isset($daily_data[$d])) {
                                                echo "<td class='text-center'>" . $daily_data[$d]['muscle_strength'] . "</td>";
                                            }
                                        }
                                        ?>
                                    </tr>
                                    <tr>
                                        ...
                                        foreach ($period as $date) {
                                            $d = $date->format('Y-m-d');
                                            if (isset($daily_data[$d])) {
                                                echo "<td class='text-center'>" . $daily_data[$d]['muscle'] . "</td>";
                                            }
                                        }
                                        ?>
                                    </tr>
                                    <tr>
                                        ...
                                        foreach ($period as $date) {
                                            $d = $date->format('Y-m-d');
                                            if (isset($daily_data[$d])) {
                                                echo "<td class='text-center'>" . $daily_data[$d]['balance'] . "</td>";
                                            }
                                        }
                                        ?>
                                    </tr>
                                    <tr>
                                        ...
                                        foreach ($period as $date) {
                                            $d = $date->format('Y-m-d');
                                            if (isset($daily_data[$d])) {
                                                echo "<td class='text-center border'>" . $daily_data[$d]['bilateral'] . "</td>";
                                            }
                                        }
                                        ?>
                                    </tr>
                            ...
                            <div class="modal-dialog modal-xl">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="weeklyPdfModalLabel">Weekly Progress Report Running Notes</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <iframe src="pdf-weekly.php?id=<?= htmlspecialchars($learnerID) ?>&start=<?= htmlspecialchars($start_date) ?>&end=<?= htmlspecialchars($end_date) ?>" 
                                                width="100%" 
                                                height="600px" 
...
var chart = new CanvasJS.Chart("chartContainer", {
	...
	data: [{
      ...
      // tentative palang ito hindi lahat nakukuha eh
			{ label: "Sensory Integration", y: <?= number_format(round(($row['walking'] + $row['jumping'] + $row['crawling']+ $row['hopping']+ $row['muscle_strength']+ $row['muscle']+ $row['balance']+ $row['bilateral']) / 8, 2) )?>},
